...
public class ServiceChromOpt implements IChromOperation {
	...
	// +
	public ServiceChromOpt(DataOparation dataOparation) {
		random = new Random();
		parameter = new GAParameter();
		dataOpt = dataOparation;
	}

	public ServiceChromOpt(GAParameter parameter, Random random,
	...
	public double calcuFitness(List<Double> encodes) {
		double fitness = 0;
		// 根据规则序列执行结果计算适应度()
		int N = dataOpt.getMeasure_x1().length;
		double estimated_x1[] = new double[N];
		double estimated_x2[] = new double[N];
		estimated_x1[0] = 0;
		List<Double> a = dataOpt.getParameter();
		double h = 1;
		...
		for (int j = 1; j < len; j++) {
			f1 = k.x1(estimated_x1[j - 1], estimated_x2[j - 1], a.get(0),
					a.get(1), encodes.get(j - 1));
			g1 = k.x2(estimated_x1[j - 1], estimated_x2[j - 1], a.get(2),
					a.get(3), a.get(4));
			f2 = k.x1(estimated_x1[j - 1] + h / 2 * f1, estimated_x2[j - 1] + h
					/ 2 * g1, a.get(0), a.get(1), encodes.get(j));
			g2 = k.x2(estimated_x1[j - 1] + h / 2 * f1, estimated_x2[j - 1] + h
					/ 2 * g1, a.get(2), a.get(3), a.get(4));
			f3 = k.x1(estimated_x1[j - 1] + h / 2 * f2, estimated_x2[j - 1] + h
					/ 2 * g2, a.get(0), a.get(1), encodes.get(j));
			g3 = k.x2(estimated_x1[j - 1] + h / 2 * f2, estimated_x2[j - 1] + h
					/ 2 * g2, a.get(2), a.get(3), a.get(4));
			f4 = k.x1(estimated_x1[j - 1] + h * f3, estimated_x2[j - 1] + h
					* g3, a.get(0), a.get(1), encodes.get(j));
			g4 = k.x2(estimated_x1[j - 1] + h * f3, estimated_x2[j - 1] + h
					* g3, a.get(2), a.get(3), a.get(4));
		...
		for (int j = 1; j < len; j++) {
			if (estimated_x1[j] > 45) {
				fitness = 0;
				break;
			} else if (estimated_x1[j] >= 24 && estimated_x1[j] <= 42) {
				fitness = fitness + 1d;
			}
		}
